...
if [[ ${METRIC_SERVER_HOST} == *"aws"* ]];then
sed -i "s/server_base = https:\/\/download.cloudwiz.cn\/release/server_base = https:\/\/aws-download.cloudwiz.cn\/release/g" ${agent_install_folder}/uagent/uagent.conf
fi
