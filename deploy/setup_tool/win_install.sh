...
agent_url=https://download.cloudwiz.cn/agent
# ---------------------------------------- #

# read password
echo -n "Please enter password for ${admin_user}:"
read -s passwd
echo
...
ansible-playbook win_agent_setup.yaml -v -i hosts.tmp --extra-vars "org_id=${org_id} sys_id=${sys_id} token=${token} metric=${metric} alertd=${alertd} agent_url=${agent_url}  ${update}"
